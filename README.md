...
## 当前交付：可链接框架与 ARServer

`http_framework` 是独立可链接的 C++11 HTTP 框架，提供 HTTP 解析、路由、Middleware、CORS、通用 Session、异步响应和静态资源服务。`ar_server` 位于 `WebApps/ARServer`，使用框架提供兼容的 AR 登录、场景会话、心跳和成员轮询 API。

### 境界AR全景门户

门户首页为“境界AR · 360° 全景探索”，提供八个 WebP 360° 场景。游客可浏览；登录用户可点赞、取消点赞、发布评论，并通过 HTTP 心跳参与在线人数统计。全景播放器使用本地固定的 A-Frame 1.6.0 文件 `WebApps/ARServer/www/vendor/aframe-1.6.0.min.js`，支持鼠标/触摸拖动及设备方向。
...
MYSQL_PWD="$MYSQL_PASSWORD" mysql -h "$MYSQL_HOST" -P "$MYSQL_PORT" \
  -u "$MYSQL_USER" "$MYSQL_DATABASE" < docs/sql/jingjie_ar_scene_interactions.sql
...
# 编辑 .env.arserver，并以 export 或 env 命令提供 MYSQL_PASSWORD；不要提交该文件。
set -a; . ./.env.arserver; set +a
./build-full/bin/ar_server
```

MySQL 模式要求 `MYSQL_PASSWORD`；连接地址、端口、线程数、缓存工作线程和 Redis 参数均由环境变量读取。兼容页面仍使用 query-string password，但访问日志会将该参数脱敏。AR.js 生产部署必须由反向代理提供 HTTPS；本项目当前不实现 TLS。

### 13 项实施任务完成说明

本次升级已完成。网络层仍保留原有 Multi-Reactor、`TcpServer`、`Buffer`、`TimerQueue`、日志、内存池、MySQL、Redis、`DBWorkerPool`、`SessionDAO`、`SessionCache` 与 `TwoLevelCache`；框架层在其上提供可嵌入 API，AR 应用位于 `WebApps/ARServer`。

| 任务 | 已完成内容 |
|------|------------|
| 1. HTTP 模型与解析 | 增量 HTTP/1.1 解析、请求大小限制、明确的解析错误和协议单元测试。 |
| 2. Router | 静态/动态路由、路径参数保留、重复路由拒绝和确定性匹配。 |
| 3. Middleware/CORS | 前后置中间件链、异常短路安全响应、正确的 Origin/预检 CORS 处理。 |
| 4. Dispatcher/HttpServer | 同步路由 API、异步路由 API，以及一次 `onMessage()` 排空同一缓冲区中的 pipeline 请求。 |
| 5. 通用 Session | 独立命名空间的 Session 值对象、存储、编解码、TTL 和线程安全管理器。 |
| 6. 异步响应与 DB | `AsyncResponder` exactly-once 保护；MySQL/Redis 工作移出 EventLoop；DAO 回调使用 `shared_ptr`/值语义。 |
| 7. Redis Session | 有界缓存工作池、二进制安全 Session 编码、可选 `http_framework_redis` 目标与 Redis 集成测试。 |
| 8. 静态资源 | 可复用 `StaticFileHandler`、ETag/Last-Modified/Range、部分 `sendfile()`、EPOLLOUT 和文件描述符生命周期修复。 |
| 9. ARServer 迁移 | 原页面按字节移动至 `WebApps/ARServer/www`；ARServer 不直接依赖 `Channel`、`Socket` 或 `TcpConnection`。 |
| 10. 场景协作 | 场景元数据、进入/退出、心跳、Redis presence、成员轮询；两人同场可见，30 秒无心跳自动离线。 |
| 11. 配置与错误契约 | `.env.arserver` 环境配置、认证与请求中间件、统一 API 错误、访问日志 password 脱敏；源码不含数据库密码。 |
| 12. 可安装框架 | 目标作用域 CMake、`http_framework` 安装包、`find_package` 外部消费者构建/链接/启动证明。 |
| 13. 验收与性能 | API/协作/断连/Redis 回源故障演练、ASan/UBSan、关键 TSan、性能脚本与当前 Release 性能矩阵。 |

最终验收记录：常规 CTest **19/19** 通过；外部消费者可独立链接；AR API、静态资源、双用户协作、断连客户端和 Redis 故障回源脚本通过。ASan/UBSan 完整套件通过；关键 TSan 用例在 Linux 的 `setarch -R` 地址布局 workaround 下无告警通过。性能环境、QPS、P50/P90/P99 和过载错误数见 [benchmark/benchmark.md](benchmark/benchmark.md)。

### 运行问题记录：Keep-Alive 下异步认证返回首页（已修复）

**现象：** 浏览器先访问 `/` 后在同一 HTTP/1.1 Keep-Alive 连接请求 `POST /api/auth`，认证接口错误返回 `index.html`。前端对 HTML 响应执行 JSON 解析失败，因而显示“网络错误”。单独执行一次认证 `curl` 则正常，容易误判为 MySQL 故障。

**根因与修复：** 静态首页通过异步回退处理；异步请求分发后，连接的 `HttpContext` 没有重置，下一条请求因此复用了上一条 GET 的解析状态。`HttpServer::onMessage()` 现在会在异步请求已复制完成后重置该上下文，使后续请求重新解析。

**回归验证：** [keep_alive_async_route_test.sh](tests/integration/keep_alive_async_route_test.sh) 在同一连接中顺序请求首页和认证接口，并要求第二个响应为认证 JSON。修复提交：`87bb1c4`。
...
BASE_URL=http://127.0.0.1:8080 tests/integration/ar_collaboration_test.sh
```

框架 core-only 构建不需要数据库依赖：
...
外部消费者使用 `find_package(http_framework CONFIG REQUIRED)`，再链接 `http_framework::http_framework`；可复现脚本为 `tests/integration/consumer_build_test.sh`。
...
# 完整 ARServer（自动检测 MySQL/hiredis）
cmake -S . -B build-full -DCMAKE_BUILD_TYPE=Release
cmake --build build-full --target ar_server -j2

# 仅构建可嵌入框架，不需要 MySQL 或 hiredis
cmake -S . -B build-core -DHTTP_FRAMEWORK_WITH_MYSQL=OFF -DHTTP_FRAMEWORK_WITH_REDIS=OFF
cmake --build build-core --target http_framework -j2
```
...
```bash
# 从项目根目录运行；复制示例后只在本地文件中填入测试数据库密码。
cp WebApps/ARServer/.env.example .env.arserver
# 编辑 .env.arserver：MYSQL_PASSWORD 必须是测试数据库密码，不要提交该文件。
set -a; . ./.env.arserver; set +a
./build-full/bin/ar_server

# 访问 http://127.0.0.1:8080/ 。生产 AR.js 部署需由反向代理提供 HTTPS。
```
...
```bash
# 1. 按“快速开始”的环境配置启动 ARServer
./build-full/bin/ar_server > /dev/null 2>&1 &
