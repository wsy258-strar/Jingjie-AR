...
// 防止一个线程创建多个EventLoop
thread_local EventLoop *t_loopInThisThread = nullptr; //thread_local 变量:线程专属的全局变量,所有线程共享同一个实例，一个线程修改，其他线程能看到变化。
                                                        // 生命周期：和线程绑定，线程创建时初始化，线程结束时销毁。
    ...
    , timerQueue_(new TimerQueue(this))
{
    ...
    wakeupChannel_->setReadCallback(                 // 设置wakeupfd的事件类型以及发生事件后的回调操作
        std::bind(&EventLoop::handleRead, this));  //将 EventLoop 的 handleRead 成员方法，绑定到当前 EventLoop 实例，生成一个可调用的回调对象（适配 Channel 读回调的类型）
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        pollRetureTime_ = poller_->poll(kPollTimeMs, &activeChannels_);// 进行 IO 多路复用检测的核心，底层调用 epoll_wait 等系统调用，检测epoll内核事件表中的FD是否有就绪的FD
...
// 在当前loop中执行某个用户任务回调(cb)，保证操作都在其所属的 IO 线程（loop_ 绑定的线程）中执行
...
{
    {   //智能锁对象：C++11 对互斥锁的 RAII 封装类（Resource Acquisition Is Initialization，资源获取即初始化）
        //构造时自动加锁：创建 unique_lock 对象时，会调用传入的 mutex_ 的 lock() 方法；
        //析构时自动解锁：unique_lock 对象销毁时（如出作用域），会调用 mutex_ 的 unlock() 方法；
...
//清空唤醒 FD 的缓冲区，避免重复触发读事件，是保证 EventLoop 唤醒机制正确运行的关键收尾步骤
void EventLoop::handleRead()
{
    uint64_t one = 1;
    //read () 的目的也不是 “读取有用数据”，而是 “取走这个唤醒标记，清空内核缓冲区”
...
{
    uint64_t one = 1;
    //write () 的目的不是 “传递有用数据”，而是 “往内核缓冲区放一个「唤醒标记」”
...
//执行 EventLoop 待处理任务队列 pendingFunctors_ 中的所有回调函数（这些任务由其他线程通过 queueInLoop() 提交），
// 保证所有任务在 EventLoop 绑定的线程中执行，同时最小化锁持有时间、避免死锁。
void EventLoop::doPendingFunctors()
{
    ...
    {   // 加锁：保护 pendingFunctors_ 的读写（多线程竞争）
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendingFunctors_); // 交换的方式减少了锁的临界区范围(不会阻塞其他线程调用queueInLoop()),提升效率,同时避免了死锁(faunctor也可能再调用queueInLoop())
        // 如果执行functor()在临界区内 且functor()中调用queueInLoop()就会产生死锁
        // 交换后，pendingFunctors_ 变为空，functors 持有所有待执行任务 
    }// 锁出作用域，自动解锁（临界区仅包含 swap，极短）
