...
// 有一个新用户连接，acceptor会执行这个回调操作，负责将mainLoop接收到的请求连接(acceptChannel_会有读事件发生)通过调轮询分发给subLoop去处理
void TcpServer::newConnection(int sockfd, const InetAddress &peerAddr)
{
   // 轮询算法（在线程池中） 选择一个subLoop 来管理connfd对应的channel
...
void TcpServer::removeConnectionInLoop(const TcpConnectionPtr &conn)
{
    ...
    connections_.erase(conn->name()); //把conn从ConnectionMap中移除
