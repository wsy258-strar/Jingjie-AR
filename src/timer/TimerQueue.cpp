...
//把addtimer()拆成两部分，拆分后的addTimer()只负责转发，addTimerInLoop()完成修改定时器列表的工作
//提高timequeue的线程安全性，让addTimer()调用runInLoop()，把实际工作转移到IO线程来做
                          ...
                          double interval)
{
    ...
    loop_->runInLoop(
        std::bind(&TimerQueue::addTimerInLoop, this, timer)); //this:当前 TimerQueue 实例的指针，绑定后 addTimerInLoop 才能访问实例的成员（如 timers_、timerfd_）；
                                                                    //timer:要添加的定时任务对象（Timer 实例），包含回调、到期时间、是否重复等信息。
}
