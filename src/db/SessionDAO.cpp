...
{
    if (!dbPool_ || !dbPool_->submit([userId, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(std::shared_ptr<User>());
            return;
        }
...
void SessionDAO::findUserByUsername(const std::string& username,
                                     const UserCallback& callback)
{
    if (!dbPool_ || !dbPool_->submit([username, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(std::shared_ptr<User>());
            return;
...
{
    if (!dbPool_ || !dbPool_->submit([username, passwdHash, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(0);
            return;
...
{
    if (!dbPool_ || !dbPool_->submit([token, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
        ...
        const char* sql = "SELECT id, session_token, user_id, scene_id, status, "
                          ...
                          "WHERE session_token = ? ORDER BY id DESC LIMIT 1";
        if (mysql_stmt_prepare(stmt, sql, strlen(sql)) != 0)
        {
            mysql_stmt_close(stmt);
            callback(std::shared_ptr<Session>());
...
void SessionDAO::createSession(uint64_t userId,
                                ...
                                const IdCallback& callback)
{
    if (!dbPool_ || !dbPool_->submit([userId, token, sceneId, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(0);
            return;
...
{
    if (!dbPool_ || !dbPool_->submit([sessionId, sceneId, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(false);
            return;
        ...
        const char* sql = "UPDATE sessions SET scene_id = ?, status = 1 WHERE id = ?";
        if (mysql_stmt_prepare(stmt, sql, strlen(sql)) != 0)
        {
            mysql_stmt_close(stmt);
            callback(false);
...
void SessionDAO::endSession(uint64_t sessionId,
                             const BoolCallback& callback)
{
    if (!dbPool_ || !dbPool_->submit([sessionId, callback](std::shared_ptr<MYSQL> conn) {
        if (!conn)
        {
            callback(false);
            return;
        ...
        const char* sql = "UPDATE sessions SET scene_id = '', status = 0 WHERE id = ?";
        if (mysql_stmt_prepare(stmt, sql, strlen(sql)) != 0)
        {
            mysql_stmt_close(stmt);
            callback(false);
