...
{
    // Kept for existing applications.  getMemoryPool() now guarantees this
    // initialization before the first allocation, so linked consumers cannot
    // accidentally use an uninitialized SlotSize_.
    getMemoryPool(0);
}   
...
MemoryPool& HashBucket::getMemoryPool(int index)
{
    ...
    std::call_once(initialized, []() {
        for (int i = 0; i < MEMORY_POOL_NUM; ++i)
            memoryPool[i].init((i + 1) * SLOT_BASE_SIZE);
    });
